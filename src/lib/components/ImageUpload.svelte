<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{ upload: { url: string; fileName: string } }>();
  ...
  export let id: string = 'image';
  export let required: boolean = false;
  export let extraButton: boolean = true;
  export let extraButtonText: string = 'Choose File';
  // export let 

  export let value: File | null = null;
  export let existingImageUrl: string | null = null;
  ...
  function simulateUpload() {
    ...
    const interval = setInterval(() => {
      ...
      if (uploadProgress >= 100) {
        ...
        uploadProgress = 100;
        isUploaded = true;
        // Dispatch upload event when upload is complete
        if (imageElement?.src) {
          dispatch('upload', { url: imageElement.src, fileName });
        }
      }
    }, 100);
  }
  ...
  async function handleCrop() {
    ...
    if (croppedCanvas) {
      // Update the image element with cropped image
      imageElement.src = croppedCanvas.toDataURL();
      
      // Convert cropped canvas to File and update the value
      const croppedFileName = fileName ? `cropped-${fileName}` : 'cropped-image.png';
      const croppedFile = await canvasToFile(croppedCanvas, croppedFileName);
      value = croppedFile; // Update value with cropped file
      fileName = croppedFileName; // Update filename
...
<div class="w-full py-9 bg-gray-50 rounded-2xl border-2 border-gray-300 gap-3 grid border-dashed" id={id}>
