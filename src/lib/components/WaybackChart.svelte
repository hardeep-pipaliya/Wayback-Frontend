...
<script lang="ts">
  ...
  function renderChart() {
    ...
    mergedData = years.reduce(
      (acc: number[], year) => acc.concat(data.years[year]),
      []
    );
    let yearCycle = 12;
    let opacity = 0.5;
  ...
  function setupChartInteraction(monthYear: string[], mergedData: number[]) {
    ...
    if (chart.scales && chart.scales.x && chart.chartArea) {
      ...
      selectionRect.w =
        chart.scales.x.getPixelForValue(endIndex) - selectionRect.startX;
  ...
  function updateOverlaySelectionFromInput() {
    ...
    const startIdx = monthYear.findIndex(
      (m) => m === formatDateToMMYYYY(startTime)
    );
    const endIdx = monthYear.findIndex(
      (m) => m === formatDateToMMYYYY(endTime)
    );
...
<div class="w-full max-w-6xl mx-auto p-6 space-y-8">
  ...
  <div class="bg-white rounded-lg shadow-lg p-6">
    <h2 class="text-2xl font-bold text-gray-800 mb-4">
      Wayback Machine Data Visualization
    </h2>
    ...
    >
        ...
        {#each Array(6) as _, i}
          <div class="flex flex-col space-y-4">
            <div class="card p-2 shadow-lg border rounded-xl overflow-hidden">
              <div
                class="relative flex justify-center items-center h-48 bg-gray-100"
              >
                <div
      ...
      {:else if screenshots.length > 0}
        {#each screenshots as item}
          <div class="flex flex-col space-y-4">
            <div
              class="card p-2 shadow-lg border rounded-xl overflow-hidden group"
            >
              <div class="relative">
                <img
      ...
      {:else}
        <div class="col-span-full text-center p-8 text-gray-500">
          ...
          <p>
            No screenshots available. Select a date range and click "Fetch
            Screenshots" to load data.
          </p>
        </div>
      {/if}
    </div>
