<script lang="ts">
  ...
  const testData: ConfigurationSetting[] = [
    {
      id: "1",
      name: "novita",
      description: "This is config for service usage",
      status: true,
      slug_id: "d1189ee6-f06c-4828-840e-089664a5a29a",
    },
    {
      id: "2",
      name: "retry",
      description: "This is for retry count",
      status: true,
      slug_id: "f7440013-93a0-4eeb-9f9d-5e5e99decd91",
    },
    {
      id: "3",
      name: "queue",
      description: "This is for calling queue serivce",
      status: true,
      slug_id: "d5233e91-1d36-4b25-8981-ea8c4f503685",
    },
  ];
  ...
  // Functions
  function handleStatusChange(event: CustomEvent, configId: string) {
    const { checked } = event.detail;
    configurations = configurations.map((config) =>
      config.id === configId ? { ...config, status: checked } : config
    );
    console.log(`Status changed for config ${configId}:`, checked);
  }
  ...
  function handleView(config: ConfigurationSetting) {
...
<div>
  ...
  >
    <div>
      ...
      <p class="text-xs text-gray-500">Manage your Configuration Settings.</p>
    </div>
  ...
  >
    <table class="w-full rounded-xl font-poppins">
      ...
      <tbody class="divide-y divide-gray-200 text-sm font-normal">
        {#each filteredConfigurations as config, index}
          <tr class="transition-all duration-500 hover:bg-gray-50">
            <td class="py-3.5 pl-4 whitespace-nowrap">{index + 1}</td>
            <td
              class="py-3.5 pl-4 whitespace-nowrap font-normal text-gray-900 pb-6"
              >{config.name}</td
            >
            <td
              class="py-3.5 pl-4 whitespace-nowrap font-normal text-gray-900 pb-6"
            ...
            <td class="py-3.5 pl-4 whitespace-nowrap">
              <FlipSwitch
                id="fs{config.slug_id}"
                checked={config.status}
                on:change={(event) => handleStatusChange(event, config.id)}
              />
            </td>
            <td class="py-3.5 px-4">
              <div class="flex font-poppins">
                <!-- Update -->
                <a href="/configuration/update/{config.slug_id}" class="mr-2" aria-label="Update">
                  <svg
                    width="32"
                    height="32"
                    viewBox="0 0 32 32"
