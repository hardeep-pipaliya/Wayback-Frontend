<script lang="ts">
  ...
  // Static test data
  let roles: Role[] = [
    { id: 1, name: 'Administrator', status: true },
    { id: 2, name: 'Editor', status: false }
  ];
  ...
  // Sort roles based on sortBy
  $: sortedRoles = [...filteredRoles].sort((a, b) => {
    switch (sortBy) {
      case 'name':
  ...
  // Paginated roles
  $: paginatedRoles = sortedRoles.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  // Update pagination when filtered roles change
  $: {
    pagination = {
      total_count: sortedRoles.length,
      offset: (currentPage - 1) * itemsPerPage,
      limit: itemsPerPage,
      total_pages: Math.ceil(sortedRoles.length / itemsPerPage)
    };
  }
  ...
  function handleRoleStatusChange(roleId: number, checked: boolean) {
    const roleIndex = roles.findIndex(role => role.id === roleId);
    if (roleIndex !== -1) {
      roles[roleIndex].status = checked;
  ...
  function handleEdit(roleId: number) {
    goto(`/role/update/${roleId}`);
  }

  function handleView(roleId: number) {
    goto(`/role/view/${roleId}`);
  }
...
<div>
    ...
    <!-- <div class="mt-4">
      <span class="text-sm text-gray-600">
        Showing {pagination.offset + 1} to {Math.min(pagination.offset + pagination.limit, pagination.total_count)} of {pagination.total_count} entries
      </span>
    </div> -->
              ...
              <tr class="transition-all duration-500 hover:bg-gray-50">
                ...
                <td class="py-3.5 pl-4 whitespace-nowrap">
                  <FlipSwitch 
                    ...
                    name="status-{role.id}"
                    checked={role.status}
                    on:change={(e) => handleRoleStatusChange(role.id, e.detail.checked)}
                  />
                </td>
                <td class="py-3.5 px-4">
                  <div class="flex font-poppins">
                    <button
                      ...
                      onclick={() => {
                        handleEdit(role.id);
                        console.log("Edit clicked for ID:", role.id);
                      }}
                    >
                      <svg
                        width="32"
