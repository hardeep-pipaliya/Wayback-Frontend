<script lang="ts">
  ...
  const mockTickets: Ticket[] = [
    ...
    {
      id: '3',
      ticketId: 'TICKET33894487',
      subject: 'User account access issue',
      status: 'resolved',
      priority: 'urgent',
  ...
  // Filter options
  const statusOptions = ['All', 'open', 'closed', 'pending', 'resolved'];
  ...
  function handleCreateTicket() {
    ...
    window.location.href = '/ticket/add/';
  }
...
<div class="min-h-screen bg-gray-50 p-6">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col lg:flex-row items-center justify-between gap-4 pb-6 border-b border-gray-200">
      <div>
        <h3 class="text-2xl font-bold text-gray-900">Tickets & List</h3>
        <p class="text-sm text-gray-600">Manage your tickets efficiently</p>
      </div>
      ...
      <div class="flex items-center gap-3 max-md:flex-wrap max-md:justify-center">
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-indigo-600 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors duration-200"
        >
    ...
    <!-- Table Section -->
    <div class="mt-6 bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
      {#if loading}
          ...
          <table class="w-full">
            <thead>
              <tr class="bg-gray-50 border-b border-gray-200">
                <th scope="col" class="py-3.5 pl-4 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">
                  #
                </th>
                <th scope="col" class="py-3.5 pl-4 text-left text-xs font-medium text-gray-900 uppercase tracking-wider">
                  Ticket ID
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              {#each paginatedTickets as ticket, index}
                <tr class="hover:bg-gray-50 transition-colors duration-200">
                  <td class="py-3.5 pl-4 text-sm text-gray-900">
                    {(currentPage - 1) * pageSize + index + 1}
                  </td>
                  <td class="py-3.5 pl-4 text-sm font-medium text-gray-900">
                    {ticket.ticketId}
                  </td>
                  <td class="py-3.5 pl-4 text-sm text-gray-900 max-w-xs truncate">
                    {ticket.subject}
                  </td>
                  ...
                  <td class="py-3.5 pl-4">
                    <div class="flex items-center space-x-2">
                      ...
                      <button aria-label="Edit ticket"
                        on:click={() => handleEditTicket(ticket.id)}
                        class="p-2 text-gray-600 hover:text-indigo-600 rounded-lg transition-colors duration-200"
                        title="Edit ticket"
                      >
                      <svg
                      width="32"
                    ...
                    >
                      ...
                      <!-- View Button -->
                      <button aria-label="View ticket"
                        on:click={() => handleViewTicket(ticket.id)}
                        class="p-2 text-gray-600 hover:text-blue-600 rounded-lg transition-colors duration-200"
                        title="View ticket"
                      >
                      <svg
                      width="32"
                  ...
                  <!-- Delete -->
                  <button aria-label="Delete" class="mr-2" on:click={() => handleDelete(config.id)}>
                    <svg
                      width="32"
                      height="32"
                      viewBox="0 0 32 32"
                    ...
                    >
                      ...
                      <button aria-label="Delete ticket"
                        on:click={() => handleDeleteTicket(ticket.id)}
                        class="p-2 text-gray-600 hover:text-red-600 rounded-lg transition-colors duration-200"
                        title="Delete ticket"
                      >
                      </button>
                    </div>
